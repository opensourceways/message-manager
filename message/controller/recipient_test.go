...
func (m *MockMessageRecipientAppService) GetRecipientConfig(countPerPage, pageNum int, userName string) ([]app.MessageRecipientDTO, int64, error) {
	args := m.Called(countPerPage, pageNum, userName)
	return args.Get(0).([]app.MessageRecipientDTO), args.Get(1).(int64), args.Error(2)
}

func (m *MockMessageRecipientAppService) AddRecipientConfig(userName string, cmd *app.CmdToAddRecipient) error {
	return m.Called(userName, cmd).Error(0)
}

func (m *MockMessageRecipientAppService) UpdateRecipientConfig(userName string, cmd *app.CmdToUpdateRecipient) error {
	return m.Called(userName, cmd).Error(0)
}

func (m *MockMessageRecipientAppService) RemoveRecipientConfig(userName string, cmd *app.CmdToDeleteRecipient) error {
	return m.Called(userName, cmd).Error(0)
}
...
func TestGetRecipientConfig(t *testing.T) {
	...
	// Successful case
	mockAppService.On("GetRecipientConfig", 10, 1, "testUser").Return([]app.MessageRecipientDTO{{}}, int64(1), nil)

	req, _ := http.NewRequest(http.MethodGet, "/message_center/config/recipient?count_per_page=10&page=1", nil)
	...
	// Error case
	mockAppService.On("GetRecipientConfig", 10, 1, "testUser").Return(nil, int64(0), xerrors.New("db error"))
...
func TestAddRecipientConfig(t *testing.T) {
	...
	AddRouterForMessageRecipientController(router, mockAppService)
	// Successful case
	mockAppService.On("AddRecipientConfig", "testUser", mock.Anything).Return(nil)

	reqBody := `{"name":"recipient1"}`
	req, _ := http.NewRequest(http.MethodPost, "/message_center/config/recipient", strings.NewReader(reqBody))
	req.Header.Set("Authorization", "Bearer testToken")
	...
	// Error case
	mockAppService.On("AddRecipientConfig", "testUser", mock.Anything).Return(xerrors.New("add error"))
...
func TestRemoveRecipientConfig(t *testing.T) {
	...
	// Error case
	mockAppService.On("RemoveRecipientConfig", "testUser", mock.Anything).Return(xerrors.New("delete error"))
